...
public class ShuffleMethods {
    ...
    boolean [] isNotPresent = new boolean [N]; //array to check if the number is already present in the shuffled array
    ...
    while (i < N - 1) { //loop until the shuffled array is filled except for the last number
            int numbers = rand.nextInt(N); //generate a random number between 0 and N-1
    ...
    for (int j = 0; j < N; j++) { //loop through the isNotPresent array to find the last number that is not present in the shuffled array
    ...
    return shuffled; //return the shuffled array
        ...
        System.out.println("Fisher-Yates Shuffle:"); //print the shuffled array from the shuffle method
        for (int i = 0; i < N; i++) {
